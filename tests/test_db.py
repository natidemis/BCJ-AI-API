...
from db import Database, NotFoundError, NoUpdatesError
...
@pytest.fixture
def invalid_key_data(rng):
    N = 10
    return zip([z for z in range(N)],
                [random.choice([set(),dict(),str()]) for z in range(N)],
                [rng.random(128) for _ in range(N)],
                [random.choice([range(1,N),None]) for _ in range(N)])
...
def test_valid_insert(database,rng, valid_data):
    """ 
    @db.insert()
    ------------
...
def test_invalid_insert_no_user(database,rng, valid_data):
    """ 
    @db.insert()
    ------------
    Tests for valid insert
...
def test_invalid_insert_typeError(database, rng,invalid_key_data):
    """ 
    @db.insert()
    ------------
    Tests for valid insert
    Performs: 
        - inserts user as dict,set,str
        
    """
    database.drop_table()
    database.make_table()
    database.insert_user(1)
    for _id,user_id,embeddings,batch_id in invalid_key_data:
        try:
            database.insert(_id=_id,
                        user_id=user_id,
                        embeddings=embeddings,
...
def test_invalid_insert_duplicate_key(database, rng, duplicate_data):
    """ 
    @db.insert()
    ------------
    Tests for valid insert
...
def test_invalid_insert_user_duplicate_key(database, rng):
    """ 
    @db.insert_user()
    ------------
...
def test_invalid_insert_user_typeError(database, rng):
    """ 
    @db.insert_user()
    ------------
    Tests for TypeError
...
def test_invalid_insert_batch(database,invalid_key_data):
    """ 
    @db.insert_batch()
    ------------
    Tests for valid insert
    Performs:
        - inserting random invad input
    """
    database.drop_table()
    database.make_table()
    data = []
    for _id,user_id,embeddings,batch_id in invalid_key_data:
        try:
            ...
            assert True
        data.append((_id,user_id,embeddings,batch_id))
    try:
        database.insert_batch(data)
        assert False
    except:
...
def test_valid_all_updates(database, valid_data,rng):
    ...
    """
    test_valid_insert(database,rng,valid_data)
...
def test_updates_no_user(database,valid_data,rng):
    """
    @db.update()
    ------------
...
def test_delete_valid(database,valid_data,rng):
    """
    @db.delete()
...
def test_delete_invalid(database,valid_data,rng):
    """
    @db.delete()
